...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        ##############################
        self.reward_semi_sparse = True
        self.semi_spase_reward_died1 = 10
        self.semi_spase_reward_died2 = -1
        self.reward_time = -0.1
        self.reward_scale = False

        print("##############  the modified reward setting ###################################")
        print("dead reward when an enemy dies: ", self.semi_spase_reward_died1)
        print("dead reward when an ally dies: ", self.semi_spase_reward_died2)
        print("time-penalty reward : ", self.reward_time)
        print("###############################################################################")
        # Other
    ...
    def step(self, actions):
        ...
        # print("##################################")
        # print("dead_allies : ", dead_allies)
        # print("dead_enemies : ", dead_enemies)
        ...
        # print("self.reward_win: ", self.reward_win)
        # print("self.max_reward : ", self.max_reward)
        # print("self.reward_scale_rate : ", self.reward_scale_rate)
    ...
    def reward_battle(self):
        ...
        # update deaths
        if not self.reward_semi_sparse:
            for al_id, al_unit in self.agents.items():
            ...
            for al_id, al_unit in self.agents.items():
                if not self.death_tracker_ally[al_id]:
                    if al_unit.health == 0:
                        # just died
                        if self.previous_ally_units[al_id].health + self.previous_ally_units[al_id].shield != 0:
                            reward += self.semi_spase_reward_died2

            for e_id, e_unit in self.enemies.items():
                if not self.death_tracker_enemy[e_id]:
                    if e_unit.health == 0:
                        if self.previous_enemy_units[e_id].health + self.previous_enemy_units[e_id].shield != 0:
                            reward += self.semi_spase_reward_died1
